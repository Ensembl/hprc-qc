...
def main():
    ...
    for acc in accessions:
        ...
        rbh_biotype = dict(zip(rbh['pair_key'], rbh.get('ensembl_biotype', pd.Series(['']*len(rbh))).map(group_biotype)))
        ...
        tc = tc[tc['pair_key'].isin(rbh_pass_keys)]
        tc_rate = tc.set_index('pair_key')['transcript_concordance_rate'].astype(float)
        ...
        # Build link rows for all RBH pairs that have divergence info
        pair_keys = [k for k in rbh['pair_key'] if k in dv.index]
        for k in pair_keys:
            rbh_status = 'pass' if k in rbh_pass_keys else 'fail'
            txc = None
            if k in tc_rate.index:
                v = float(tc_rate[k])
                txc = 'full' if v >= 1.0 else ('partial' if v > 0 else 'none')
            bio = rbh_biotype.get(k, 'other')
            ...
            links_rows.append({
                ...
                'sample_name': sample,
                'pair_key': k,
                'ensembl_gene_id': k.split('|')[0],
                'cat_gene_id': k.split('|')[1],
                'gene_name': gene_name,
        ...
        # Per-assembly flows for A/B/C
        # A: RBH(pass/fail) → Concordance(full/partial/none for pass only) → Biotype → GRCh38 category
        # We summarise adjacent pairs only; the notebook will draw ribbons between successive rungs.
        df_links = pd.DataFrame([r for r in links_rows if r['assembly_accession'] == acc])
        if df_links.empty:
            continue
