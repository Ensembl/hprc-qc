process FETCH_CAT_GFF {
    ...
    # Prefer new *_consensus.gff3.gz files if present (as of Feb 2026)
    for SAMPLE_VAR in \${SAMPLE_NAMES}; do
        for PATTERN in "\${SAMPLE_VAR}_consensus.gff3.gz" "\${SAMPLE_VAR}_CAT_v2.0.gff3.gz"; do
            CAT_FILE="\${PATTERN}"
            URL="\${BASE_URL}/\${CAT_FILE}"
            echo "Trying: \${URL}" >&2
            if curl -fsSL -o ${sample_name}_cat.gff3.gz "\${URL}" 2>/dev/null; then
                if [ -s "${sample_name}_cat.gff3.gz" ]; then
